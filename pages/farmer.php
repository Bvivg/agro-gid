...
$pageTitleValue = "Фермер " . $userInfo['username'];
$pageTitle = '<p class="title-block">' . $pageTitleValue . '</p>';
$title = "Фермер " . $userInfo['username'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SESSION["user_id"])) {
  ...
  if (isset($_POST['hire'])) {
    ...
    $userId = $_GET['id'];
    $stmtHire = $connect->prepare("INSERT INTO hiring (sender_id, receiver_id, hire_date, hire_time, status) VALUES (:senderId, :receiverId, CURRENT_DATE, CURRENT_TIME, 0)");
    $stmtHire->bindParam(':senderId', $senderId, PDO::PARAM_INT);
    $stmtHire->bindParam(':receiverId', $receiverId, PDO::PARAM_INT);
...
<div class="worker">
  ...
  <div class="worker-card">
    <div class="worker-info">
      ...
      <?php
      $stmtCheckHiring = $connect->prepare("SELECT COUNT(*) FROM hiring WHERE sender_id = :senderId AND receiver_id = :receiverId");
      $stmtCheckHiring->bindParam(':senderId', $user_id, PDO::PARAM_INT);
      $stmtCheckHiring->bindParam(':receiverId', $userInfo['id'], PDO::PARAM_INT);
      $stmtCheckHiring->execute();
      $hasHiringRequest = $stmtCheckHiring->fetchColumn();
      ?>
      <form method="post" class="" style="padding: 10px 2px; display:flex; flex-direction:column; gap: 5px; text-align: center;">
        <?php if ($hasHiringRequest) : ?>
          <span class="button-modal">Заявка оформлена</span>
        <?php else : ?>
          <button type="submit" class="button-modal" name="hire">Трудоустроиться</button>
        <?php endif; ?>
        <a class="button-modal-cancel" style="" href="chat?id=<?php echo $userInfo['id']; ?>">Связаться</a>
      </form>
    ...
    <?php foreach ($feedbackInfo as $feedback) : ?>
      <div class="feedback-item" id="feedback_<?php echo $feedback['id']; ?>">
        <div class="feedback-avatar">
          <?php if ($feedback['sender_avatar_path'] && file_exists($feedback['sender_avatar_path'])) : ?>
