...
try {
  $stmtCheckJobRequest = $connect->prepare("
    SELECT hiring.*, users.username, users.email, users.avatar_path
    FROM hiring
    JOIN users ON hiring.sender_id = users.id
    WHERE hiring.receiver_id = :receiverId AND hiring.status = 0
  ");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  if (isset($_POST["accept"])) {
    ...
    $updateStatusQuery = "UPDATE hiring SET status = 1 WHERE id = :requestId";
    $stmtUpdateStatus = $connect->prepare($updateStatusQuery);
    $stmtUpdateStatus->bindParam(':requestId', $requestId, PDO::PARAM_INT);
    $stmtUpdateStatus->execute();
  } elseif (isset($_POST["reject"])) {
    $requestId = $_POST["request_id"];
    $updateStatusQuery = "UPDATE hiring SET status = 2 WHERE id = :requestId";
    $stmtUpdateStatus = $connect->prepare($updateStatusQuery);
    $stmtUpdateStatus->bindParam(':requestId', $requestId, PDO::PARAM_INT);
    $stmtUpdateStatus->execute();
  }
